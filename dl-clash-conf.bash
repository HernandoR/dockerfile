...
# merges OverWriteFile.yml into ConfFile.yml (inplace)
yq eval-all 'se. as $item ireduce ({}; . *+ $item)' $ConfFile.yml $OverWriteFile.yml

if [ $? -ne 0 ];
then
  echo "config file merge fail"
  exit $?
fi
if [[ -f "$ConfPath/postConf.sh" ]];
then
  bash $ConfPath/postConf.sh $ConfFile
fi
